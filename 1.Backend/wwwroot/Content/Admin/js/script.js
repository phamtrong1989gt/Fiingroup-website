...
var thisAcction = GetAccionByUrl(window.location.pathname);
...
function initMenuAdmin() {
    $("#menuleftadmin li").each(function (i, item) {
        if (thisController != "") {
            ...
            if (thisController == GetControllerByUrl(thisa.attr("href")) && thisAcction == GetAccionByUrl(thisa.attr("href"))) {
...
$.fn.toGridView = function (isFirst, page, callBackEndDefail = null, callBackGetValue = null, callBackSetValue = null, callbackSelectAction = null, callBack = null) {
    ...
    if (isFirst) {
        ...
        var topTable = $parent.find("table").offset().top;
        var hightPaging = $parent.find(".divpagination").height();
        var height = fullHeight - topTable - hightPaging - 37;
    }

    //$parent.find(".table-responsive").css({ "height": "" + height + "px", "overflow": "hidden" });
    //$parent.find("table").find("tbody").css({ "height": "" + height + "px" });
    ...
    $.ajax({
        ...
        if (callBack == null) {
            ...
            initScrollBodyTable($parent, isFirst);
...
$.fn.toDateTimePickerGlobal = function () {
...
$.fn.toRangeDateTimePicker = function () {
...
function removeFileUpload(e) {
    ...
    $.ajax({
        ...
        error: function () {
            $this.button('reset');
            alert("Phiên làm việc kết thúc do không thao tác một khoảng thời gian, vui lòng F5 lại trình duyệt");
        },
        complete: function (jqxhr, txt_status) {
            if (jqxhr.status == 502) {
                $this.button('reset');
...
function initFileString(currentFiles, parent) {

    var strFile = "";
    for (i = 0; i < currentFiles.length; i++) {
        strFile += `<div class='col-md-3'><span  onclick="remoteItemFileupload('` + parent + `','`+i+`')" class="btXoaImage">Xóa</span><a title="Xem chi tiết" target="_blank" href='` + currentFiles[i].path + `'><img src="` + currentFiles[i].path +`" style="width:100%" /></a></div>`;
    }
    if (currentFiles.length == 0) {
        return "<div class='col-md-12'><span>Chưa có tệp nào tải lên</span></div>";
    }
    else {
        return strFile;
    }
...
function remoteItemFileupload(parent, i) {
    var dataJson = [];
    try {
        var data = $(parent +" [data-input-form]").val();
        dataJson = JSON.parse(data);
        console.log(dataJson);
    } catch {}
    dataJson.splice(i, 1);
    $(parent + " [data-list-files]").html(initFileString(dataJson, parent));
    $(parent + " [this-data-current-files]").attr("data-current-files", JSON.stringify(dataJson));
    $(parent + " [data-input-form]").val(JSON.stringify(dataJson));
}

$.fn.toUploadFile = function (callbackUrl) {

    $(document).on("change", "[file-input-single]", function () {
        var strParent = this;
        var thisInput = $(this);
        ...
        if (thisInput.attr("multiple") != undefined) {
            ...
            if (fileSize > fileMaxSize) {
                ...
                parentInput.find("[data-add]").show();
                parentInput.find("[data-dang-tai]").hide();
                return;
            }
        ...
        if (callbackUrl != null) {
            apiUrl = callbackUrl(apiUrl);
        }

        parentInput.find("[data-add]").hide();
        parentInput.find("[data-dang-tai]").show();

        uploadFile(apiUrl, fileUpload.files[0], 0, parentInput.find("[progress-group]")[0], (thisInput.attr("multiple") == undefined ? false : true), function (rs) {
            if (rs != null) {
                setTimeout(function () {
                    if (rs.output == 1) {
                        var newSrc = rs.data.path + "?v=" + Math.random();
                        parentInput.find("[file-image]").attr("style", "background-image: url('" + newSrc + "')");
                        parentInput.find("[data-input-form]").val(rs.data.path);
                        parentInput.find("[data-add]").show();
                        parentInput.find("[data-dang-tai]").hide();
                    }
                }, 300);
    ...
    $(document).on("change", "[file-input]", function () {
        ...
        try {
            var strDataX = thisInput.attr("data-current-files");
            if (strDataX == null || strDataX == '') {
                currentFiles = [];
            }
            else {
                currentFiles = JSON.parse(strDataX);
        ...
        for (var i = 0; i < fileUpload.files.length; i++) {
            uploadFile(apiUrl, fileUpload.files[i], i, parentInput.find("[progress-group]")[0], (thisInput.attr("multiple") == undefined ? false : true), function (rs) {
                if (rs != null) {
                    setTimeout(function () {
                        if (rs.output == 1) {
                            if (type == "image") {
                            ...
                            currentFiles.push(rs.data);
                            parentInput.find("[data-list-files]").html(initFileString(currentFiles, parentInput.attr("data-parent")));
                            thisInput.attr("data-current-files", JSON.stringify(currentFiles));
                            parentInput.find("[data-input-form]").val(JSON.stringify(currentFiles));
                            // parentInput.find("[file-name]").html("<strong>Tên tệp đính kèm:</strong> <span class='text-w500'>" + rs.data.split(/(\\|\/)/g).pop() + "</span>");
    ...
    $(document).on("click", "[file-button]", function () {
        var $this = $(this);
        try {
            $($this.attr("data-tag-parrent")).find("[file-input]")[0].click();
        } catch {
            $($this.attr("data-tag-parrent")).find("[file-input-single]")[0].click();
        }
...
function initScrollBodyTable(parrent, isFirst) {
    // Lấy phần tử .table-responsive trong parent
    parrent = $(parrent);
    const tableContainer = parrent.find('.table-responsive');
    if (!tableContainer.length) return; // Nếu không tìm thấy, thoát hàm

    // Lấy vị trí top của .table-responsive so với viewport
    const tableTop = tableContainer.offset().top;

    // Lấy chiều cao của viewport
    const viewportHeight = $(window).height();

    // Tính chiều cao tối đa cho bảng
    const maxHeight = viewportHeight - tableTop - 75; // 20 là khoảng cách padding hoặc margin (nếu cần)
    tableContainer.css('max-height', maxHeight + 'px'); // Áp dụng max-height cho .table-responsive
    tableContainer.css('min-height', maxHeight + 'px'); // Áp dụng max-height cho .table-responsive

    // Nếu là lần đầu tiên, đồng bộ chiều rộng của các cột giữa thead và tbody
    if (isFirst) {
        const tHeadTh = tableContainer.find('thead tr th');
        const tbodyTr = tableContainer.find('tbody tr:first-child td');

        // Đồng bộ chiều rộng của các cột
        setTimeout(() => {
            for (let i = 0; i < tbodyTr.length; i++) {
                const width = $(tbodyTr[i]).outerWidth();
                $(tHeadTh[i]).outerWidth(width);
            }
        }, 1);
    }
}
...
window.addEventListener('load', initScrollBodyTable);
window.addEventListener('resize', initScrollBodyTable);
