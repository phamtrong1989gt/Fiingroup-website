...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	 ...
	 */
	var defaults = {
        baseUrl: '/Content/Admin/plugins/richfilemanager/',	// relative path to the FM plugin folder
	...
	var configure = function() {
		return $.when(loadConfigFile('default'), loadConfigFile('user')).done(function(confd, confu) {
			...
			if(config.api.connectorUrl) {
                ...
                var connectorUrl = "/admin/base/filemanager";
	...
	var initialize = function () {
		...
		configSortField = chunks[0] || 'created';
		configSortOrder = chunks[1] || 'desc';
	...
	var FmModel = function() {
        ...
        var TableViewModel = function () {
            ...
            this.thCreated = new SortableHeader('created');
            ...
            this.copyUrl = function (text) {
                ...
                if (window.clipboardData && window.clipboardData.setData) {
                    ...
                    try {
                        ...
                        fm.success("Copy thành công bấm tổ hợp phím Ctrl + v để ghi dữ liệu.");
                        return true;
                    } catch (ex) {
                        return false;
                    } finally {
		...
		function getSortSubject(item) {
			...
			switch(sortField) {
				...
				case 'modified':
                    ...
                    break;
                case 'created':
                    sortBy = item.rdo.attributes.created;
                    break;
				case 'dimensions':
    ...
    var selectItem = function (resourceObject) {
        ...
        if (_url_.param('field_input')) {
            window.opener.document.getElementById(_url_.param('field_input')).value = previewUrl;
            try {
                window.opener.document.getElementById(_url_.param('field_img')).src = previewUrl;
            } catch (e) {
            }
        }
		...
		if(config.upload.multiple) {
			...
			$uploadButton.unbind().click(function() {
                ...
                $dropzone.on('click', '.button-start', function (e) {
                    ...
                    //that.options.formData.dataSize = $("#zipSelect").val();
