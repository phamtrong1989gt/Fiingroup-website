        ...
        public async Task<ResponseModel> ContactPost(ContactHomeModel use, string language)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // Extract emails from products for CC (will be merged with support@fiingroup.vn automatically)
                    var productCCEmails = producs
                        .Where(p => !string.IsNullOrWhiteSpace(p.Emails))
                        .Select(p => p.Emails.Trim())
                        .Where(email => !string.IsNullOrWhiteSpace(email))
                        .Distinct()
                        .ToList();

                    var productCCString = productCCEmails.Any() ? string.Join(",", productCCEmails) : null;

                    // Send Email to customer (không làm ảnh hưởng flow chính)
                    // Load template và gửi email - lỗi sẽ được log nhưng không throw
                    var templateHtml = await LoadEmailTemplateAsync("ConsultationRequest", language);
        ...
        public async Task<ResponseModel> ContactSolutionPost(ContactSolotionModel use, string language)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // Extract emails from products for CC (will be merged with support@fiingroup.vn automatically)
                    var productCCEmails = producs
                        .Where(p => !string.IsNullOrWhiteSpace(p.Input1))
                        .Select(p => p.Input1.Trim())
                        .Where(email => !string.IsNullOrWhiteSpace(email))
                        .Distinct()
                        .ToList();
